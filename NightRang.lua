...
local statusrt = redis:get(bot_id.."NightRang:Add:Validity:Group:Rt"..mohammed..msg.chat_id_)
...
if text == ("الغاء كتم العام") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_Bots(msg) then
function FunctionStatus(arg, result)
redis:srem(bot_id.."NightRang:Removal:User:Groups", result.sender_user_id_)
...
if text == ("فك تقييد") and tonumber(msg.reply_to_message_id_) ~= 0 and Admin(msg) then
function FunctionStatus(arg, result)
if msg.can_be_deleted_ == false then 
send(msg.chat_id_, msg.id_,"عذرآ البوت ليس ادمن") 
return false  
...
Send_Options(msg,result.sender_user_id_,"reply","تم فك تقييده")  
end
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, FunctionStatus, nil)
end
...
if text and text:match("^كتم عام @(.*)$") and Dev_Bots(msg) then
function FunctionStatus(arg, result)
if (result.id_) then
if result and result.type_ and result.type_.ID == ("ChannelChatInfo") then
send(msg.chat_id_,msg.id_,"عذرا هاذا معرف قناة")   
...
if text and text:match("^الغاء كتم العام @(.*)$") and Dev_Bots(msg) then
function FunctionStatus(arg, result)
if (result.id_) then
...
if text and text:match("^فك تقييد @(.*)$") and Admin(msg) then
function FunctionStatus(arg, result)
if msg.can_be_deleted_ == false then 
send(msg.chat_id_, msg.id_,"عذرآ البوت ليس ادمن") 
return false  
...
Send_Options(msg,result.id_,"reply","تم فك تقييده")  
else
send(msg.chat_id_, msg.id_,"المعرف غلط ")
end
end
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^فك تقييد @(.*)$")}, FunctionStatus, nil)
end
...
if text == ("قائمة العام") and Dev_Bots(msg) or text == ("المحظورين عام") and Dev_Bots(msg) then
local list = redis:smembers(bot_id.."NightRang:Removal:User:Groups")
if #list == 0 then
return send(msg.chat_id_, msg.id_,"• لا يوجد محظورين عام")
end
...
if text == "المنشئين الاساسيين" and DeveloperBot(msg) or text == "الاساسين" and DeveloperBot(msg) then
...
if text == ("@all") and Admin(msg) then
...
if text and text:match('@all (.*)') and Admin(msg) then
tdcli_function({ID="GetChannelFull",channel_id_ = msg.chat_id_:gsub('-100','')},function(argg,dataa) 
tdcli_function({ID = "GetChannelMembers",channel_id_ = msg.chat_id_:gsub('-100',''), offset_ = 0,limit_ = dataa.member_count_
},function(ta,NightRang)
x = 0
...
if text == ("مسح قائمة العام") and Dev_Bots(msg) or text == ("مسح المحظورين عام") and Dev_Bots(msg) then
redis:del(bot_id.."NightRang:Removal:User:Groups")
send(msg.chat_id_, msg.id_, "• تم مسح المحظورين عام من البوت")
end
