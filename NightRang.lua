...
local url , res = https.request('https://api-alshh.cf/sub.php'..User..'') 
...
send(msg.chat_id_,msg.id_,'\nاهلا عزيزي المستخدم لا يمكنك استخدم الامر يجب الاشتراك في \n قناة البوت \n- @hlil3')   
return false
end
if not redis:get(bot_id..'NightRang:Cheking:Seted'..msg.chat_id_) and not Owner(msg) then
...
if text == 'نقل بويكا' and Dev_Bots(msg) then
local Users = redis:smembers('NightRang:'..bot_id.."userss")
local Groups = redis:smembers('NightRang:'..bot_id..'groups') 
for i = 1, #Groups do
redis:sadd(bot_id..'NightRang:Chek:Groups',Groups[i])  
local list1 = redis:smembers('NightRang:'..bot_id..'creatorbasic:'..Groups[i])
for k,v in pairs(list1) do
redis:sadd(bot_id.."NightRang:Basic:Constructor"..Groups[i], v)
end
local list2 = redis:smembers('NightRang:'..bot_id..'creator:'..Groups[i])
for k,v in pairs(list2) do
redis:sadd(bot_id.."NightRang:Constructor"..Groups[i], v)
end
local list3 = redis:smembers('NightRang:'..bot_id..'owners:'..Groups[i])
for k,v in pairs(list3) do
redis:sadd(bot_id.."NightRang:Manager"..Groups[i], v)
end
local list4 = redis:smembers('NightRang:'..bot_id..'mods:'..Groups[i])
for k,v in pairs(list4) do
redis:sadd(bot_id.."NightRang:Mod:User"..Groups[i], v)
end
redis:set(bot_id.."NightRang:Lock:tagservrbot"..Groups[i],true)   
list ={"Lock:Bot:kick","Lock:User:Name","Lock:hashtak","Lock:Cmd","Lock:Link","Lock:forward","Lock:Keyboard","Lock:geam","Lock:Photo","Lock:Animation","Lock:Video","Lock:Audio","Lock:vico","Lock:Sticker","Lock:Document","Lock:Unsupported","Lock:Markdaun","Lock:Contact","Lock:Spam"}
for i,lock in pairs(list) do 
redis:set(bot_id..'NightRang:'..lock..Groups[i],"del")    
end
end
for i = 1, #Users do
redis:sadd(bot_id..'NightRang:UsersBot',Users[i])  
end
...
local f = io.open('Links - @hlil3.txt', 'w')
f:write(test)
f:close()
sendDocument(msg.chat_id_, msg.id_,'./Links - @hlil3.txt', '\nLinks - @hlil3.txt')
end
...
if text == "تنظيف الميديا" and Admin(msg) then   
NightRang = {[0]=msg.id_}
local Message = msg.id_
for i=1,100 do
Message = Message - 1048576
...
tdcli_function({ID = "GetMessages",chat_id_ = msg.chat_id_,message_ids_ = NightRang},function(arg,data)
new = 0
NightRang2 = {}
for i=0 ,data.total_count_ do
if data.messages_[i] and data.messages_[i].content_ and data.messages_[i].content_.ID ~= "MessageText" then
NightRang2[new] = data.messages_[i].id_
new = new + 1
end
end
Delete_Message(msg.chat_id_,NightRang2)
end,nil)  
